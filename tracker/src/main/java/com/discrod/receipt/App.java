...
public class App extends ListenerAdapter
{
    ...
    public void onPrivateMessageReceived(PrivateMessageReceivedEvent event) {
        if(!event.getAuthor().isBot()){
            ...
            if(message.equalsIgnoreCase("!exit") || message.equalsIgnoreCase("!q") || message.equalsIgnoreCase("!quit"))
                System.exit(1);
            if(start_inputs){
                collectReceiptData(message, event);
            }
    ...
    private static void collectReceiptData(String message, PrivateMessageReceivedEvent event){
        //collect each value for the receipt object
        if(store_name == null){
            store_name = message;
            sendMessage(event, "Enter the amount spent please");
        }
        else if(total_amount == null){
            total_amount = message.replaceAll("[$]| ", "");
            ...
            if(!message.equalsIgnoreCase("no") || !event.getMessage().getAttachments().isEmpty()){
                downloadImgFromAttachment(event);
            }
            start_inputs = false;
            try{
                runData();
    ...
    private static void runData() throws Exception{
        ...
        public static void downloadImgFromAttachment(PrivateMessageReceivedEvent event){
            try{
